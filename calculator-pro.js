function calculatorPro() 
{
     ...
     for (var i=0;i<numberList.length;i++)
      ...
      console.log("The result of the sum is "+ results[0].toFixed(3).replace(/(\.0*|(?<=(\..*))0*)$/, '')+ 
      "\r\nThe result of the rest is "+ results[1].toFixed(3).replace(/(\.0*|(?<=(\..*))0*)$/, '')+ 
      "\r\nThe result of the multiplication is "+ results[2].toFixed(3).replace(/(\.0*|(?<=(\..*))0*)$/, '')+
      "\r\nThe result of the division is "+ results[3].toFixed(3).replace(/(\.0*|(?<=(\..*))0*)$/, ''));
    }
